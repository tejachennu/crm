...
const getUserData = async (userId) => {
  try {
    ...
    const userPlatforms = user.platform ? user.platform.split(",").map((platform) => platform.trim()) : []
    ...
    // Add "facebook Reels" if "facebook" exists in userPlatforms
    ...
    // Match user platforms with defined APIs
    const validPlatforms = apis.filter((api) => userPlatforms.includes(api.name))
  ...
  try {
    ...
    const fetchPromises = userPlatforms.map(async (platform) => {
      try {
        ...
        console.error(`Error fetching posts for platform: ${platform.name}, blogId: ${platform.blogId}`, error.message)
      }
    })
...
const processSinglePost = async (post, blogId, platformId, model, userId) => {
  try {
    // Check if the post already exists in the database
    const postIdField = model === FbReels || model === IgReels ? "reelId" : "postId"
    const existingPost = await model.findOne({ where: { [postIdField]: post[postIdField] } })

    if (existingPost) {
      return // Post already exists, skip processing
    }

    // Construct postData dynamically based on the platform and content type
    let postData
    if (model === FbPosts) {
      postData = {
        platformId,
      ...
      postData = {
        ...
        blueReelsPlayCount: post.blueReelsPlayCount || 0,
        postImpressionsUnique: post.postImpressionsUnique || 0,
        postVideoAvgTimeWatchedSeconds: post.postVideoAvgTimeWatchedSeconds || 0,
        postVideoViewTimeSeconds: post.postVideoViewTimeSeconds || 0,
        postVideoReactions: post.postVideoReactions || 0,
        postVideoSocialActions: post.postVideoSocialActions || 0,
        engagement: post.engagement || 0,
        reach: post.reach || 0,
        comments: post.comments || 0,
      }
    } else if (model === IgReels) {
      postData = {
        platformId,
        userId,
        businessId: post.businessId,
    ...
    await model.create(postData)
    console.log(
      `${model === FbReels || model === IgReels ? "Reel" : "Post"} added for blogId: ${blogId}, ${postIdField}: ${post[postIdField]}`,
    )
  } catch (error) {
    console.error(
      `Error processing ${model === FbReels || model === IgReels ? "reelId" : "postId"}: ${post[model === FbReels || model === IgReels ? "reelId" : "postId"]}`,
      error.message,
    )
  }
}
