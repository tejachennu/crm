  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const formattedData = Object.entries(result.dailyStats).map(([date, metrics]: [string, any]) => ({
          date,
          ...metrics,
        }))
    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{totalStats.totalEngagement.toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{totalStats.avgWatchTime.toFixed(2)}s</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{totalStats.totalReach.toLocaleString()}</div>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data}>
                ...
                <YAxis />
                <Tooltip />
                <Line type="monotone" dataKey="blueReelsPlayCount" stroke="#2563eb" name="Views" />
      ...
      <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Line type="monotone" dataKey="postVideoReactions" stroke="#dc2626" name="Reactions" />
                  <Line type="monotone" dataKey="comments" stroke="#9333ea" name="Comments" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Unique impressions and total reach</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Line type="monotone" dataKey="postImpressionsUnique" stroke="#ea580c" name="Unique Impressions" />
                  <Line type="monotone" dataKey="reach" stroke="#0891b2" name="Reach" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
