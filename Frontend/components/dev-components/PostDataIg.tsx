...
const InstagramPostData = ({ postId }: { postId: string }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          `https://api.aquarythu.com/api/IGPDtTrk/${postId}`
        );
  ...
  if (!data.length) {
    return (
      <Card className="w-full max-w-md mx-auto">
        ...
        <CardContent>
          <p className="text-gray-500">There is no data available for this post. Old post data may not be available.</p>
        </CardContent>
      </Card>
    )
  }
      ...
      <div className="bg-white rounded-lg shadow-md">
        <div className="p-4 mx-auto overflow-x-auto max-w-7xl">
          <Table className="w-full">
            <TableHeader className="text-white bg-pink-600">
              <TableRow>
